...
Run `<yaps> status --pretty`, `<yaps> auth status --redact`, and `<yaps> features list --pretty` as appropriate. Inspect the installed group's `--help` before invoking it. Never invent a command or assume a feature, model, language, or account entitlement is available.
